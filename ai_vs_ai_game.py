...
class ShootingGame:
    ...
    def ai1_shoot_bullet(self):
        if self.ai1_bullet_cooldown == 0:
            ...
            # ai2_x, ai2_y = self.ai2_position
            # ai1_x, ai1_y = self.ai1_position
            ...
            # self.ai1_reward += (sum(dis_matrix * direction_matrix * direction_label_matrix))**2
    ...
    def ai2_shoot_bullet(self):
        if self.ai2_bullet_cooldown == 0:
            ...
            # self.ai2_reward += (sum(dis_matrix * direction_matrix * direction_label_matrix))**2
    ...
    def check_bullet_collisions(self):
        ...
        for i, bullet in enumerate(self.ai1_bullets):
            ...
            if bullet_rect.colliderect(
                pygame.Rect(*self.ai2_position, self.ai_size, self.ai_size)
            ):
    ...
    def step(self, input1, input2, dis_reward_alpha):
        self.ai1_reward = 0
        self.ai2_reward = 0

        action1 = np.zeros(6, int)
        action1[input1] = 1

        action2 = np.zeros(6, int)
        action2[input2] = 1
        ...
        dis_x = abs(self.ai2_position[0] - self.ai1_position[0]) / 64
        dis_y = abs(self.ai2_position[1] - self.ai1_position[1]) / 64

        self.ai1_reward -= (dis_x * dis_y) * dis_reward_alpha
        self.ai2_reward -= (dis_x * dis_y) * dis_reward_alpha

        self.ai1_reward -= 0.02
        self.ai2_reward -= 0.02
